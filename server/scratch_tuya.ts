...
async function discover() {
  try {
    console.log("Fetching devices for project...");
    ...
    if ((res as any).result && (res as any).result.devices && (res as any).result.devices.length > 0) {
        const deviceId = (res as any).result.devices[0].id;
